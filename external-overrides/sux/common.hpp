...
#include <x86intrin.h>
#else
#warning "Skipping x86intrin.h on non-x86 architecture"
#endif
 ...
 */
 inline uint64_t select64(uint64_t x, uint64_t k) {
        ...
        // x86_64 com GCC ou Clang — usa instruções PDEP + TZCNT via asm
        ...
        // x86_64 com outro compilador (tipo MSVC) — usa intrinsics
        return _tzcnt_u64(_pdep_u64(1ULL << k, x));
    
    #else
        // Qualquer outro caso (tipo ARM) — fallback com aritmética
        ...
        uint64_t place = __builtin_ctzll(geqKStep8) & ~7ULL;
        uint64_t byteRank = k - (((byteSums << 8) >> place) & 0xFFULL);
