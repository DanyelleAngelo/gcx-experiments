...
void compress ( Char *name, char *file_report_gcx)
{
   ...
   clock_time = clock(); //mede o tempo, relatório gcx
...
#  endif
   ...
   //gcx
   char *report_gcx = NULL;
   for (aa = argList; aa != NULL; aa = aa->link) {
      if (strncmp(aa->name, "--gcx_report=", 13) == 0) {
         report_gcx = aa->name + 13;
      }
   }
   ...
   for (aa = argList; aa != NULL; aa = aa->link) {
      ...
      if (strncmp(aa->name, "--gcx_report=", 13) == 0) continue; //gcx
      numFileNames++;
   ...
   if (opMode == OM_Z) {
     if (srcMode == SM_I2O) {
        ...
        for (aa = argList; aa != NULL; aa = aa->link) {
           ...
           if (strncmp(aa->name, "--gcx_report=", 13) == 0) continue; //gcx
           numFilesProcessed++;
           compress ( aa->name, report_gcx ); //gcx
        }
     }
   } 
   else
   ...
   if (opMode == OM_UNZ) {
      ...
      if (srcMode == SM_I2O) {
         ...
         for (aa = argList; aa != NULL; aa = aa->link) {
            ...
            if (strncmp(aa->name, "--gcx_report=", 13) == 0) continue; // gcx
            numFilesProcessed++;
            uncompress ( aa->name, report_gcx ); //gcx
         }      
      }
      if (unzFailsExist) { 
         setExit(2); 
