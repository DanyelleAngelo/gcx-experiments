...
for grammar_file in arquivos_grammar:
    encoding_file = grammar_file.replace('-gcx-grammar.csv', '-gcx-encoding.csv')
    ...
    if encoding_file in arquivos_encoding:
        ...
        print(f'{encoding_file} → colunas: {arquivo_encoding.columns.tolist()}')
        arquivo_encoding_reduced = arquivo_encoding[['algorithm', 'compressed_size', 'plain_size']]
